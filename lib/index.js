...
const AGENT_OPTIONS = {
  keepAlive: true,
  keepAliveMsecs: 60e3,
  maxSockets: Infinity, // TODO investigate
  maxFreeSockets: 256,
}
...
function request(opt, done, _boundOptions) {
  opt = util.merge({
    ...
    total: 5,
    serialize: 'json',
    parseResponse: util.parseJSONResponse,
    parseError: util.parseJSONError,
    maxRedirects: 5,
    timeout: 60e3,
  }, _boundOptions, opt)
  ...
  if (opt.timeout) maxTimer = setTimeout(function () {
    done(new util.RequestError(0, 'connection reset by client'))
    abort()
  }, opt.timeout)
  if (opt.gzip) compress(opt, retry) // FIXME abort during compression
  else {
    opt.headers['content-length'] = Buffer.byteLength(opt.body, 'utf8')
    retry()
  }
  function retry() {
    attempts += 1
  ...
  emitter.on('response', function (responseEmitter) {
    ...
    if (util.contains(REDIRECT_CODES, res.statusCode)) {
      if (redirectAttempts >= opt.maxRedirects)
        return done(new util.RequestError(res.statusCode, 'too many redirects'))
    ...
    collect(responseEmitter, function (responseBuffer) {
      ...
      if (res.statusCode >= 200 && res.statusCode < 300) {
        let result = opt.parseResponse(res)
        if (result instanceof Error) done(result)
        else done(null, result)
      }
      else done(opt.parseError(res))
    })
  })
...
module.exports = util.merge(request, util, {
  ...
  default: request,
  withBindings: util.withBindings.bind(request),

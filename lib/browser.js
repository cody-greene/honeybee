...
function request(opt, done, _boundOptions) {
  ...
  function next(err, res) {
    ...
    else done(err, res) // TODO ensure this is only called once
  }
...
function perform(opt, done) {
  ...
  xhr.onload = function () {
    ...
    if (res.statusCode === 204) done()
    else if (res.statusCode >= 200 && res.statusCode < 300) {
      var result = opt.parseResponse(res)
  ...
  xhr.onerror = function () {
    done(new util.RequestError(0, 'network error'))
  }
  if (opt.onProgress) {
